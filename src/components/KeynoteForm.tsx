  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div>
        ...
        <input type="text" name="datetime" placeholder="e.g. 20 Sep, 11:00 AM" value={formData.datetime} onChange={handleChange} onFocus={() => setFocused('datetime')} onBlur={() => setFocused(null)} style={field('datetime')} />
      </div>

      <div>
        <label style={labelStyle}>Message</label>
        <textarea name="message" rows={5} placeholder="Tell us about your event, audience size, and what you're looking for." required value={formData.message} onChange={handleChange} onFocus={() => setFocused('message')} onBlur={() => setFocused(null)} style={{ ...field('message'), resize: 'none' }} />
      </div>
